...
export default function HistoricoReunioes() {
  ...
  let reunioes: ReuniaoWithTCC[] = [];
  let atualizarReunioes = async () => {};
  let contextError = null;
  try {
    const ctx = useReunioes();
    reunioes = ctx.reunioes || [];
    atualizarReunioes = ctx.atualizarReunioes;
  } catch (err) {
    contextError = err;
  }
  ...
  }, [atualizarReunioes, contextError]);
  ...
  return (
    <div className="min-h-screen bg-yellow-400 flex justify-center items-start py-10 px-4">
      <div className="bg-white max-w-2xl w-full rounded-xl p-6 space-y-6 shadow-md">
        ...
        {/* Cards, Loading, Erro e Fallback de contexto */}
        <div className="space-y-4">
          {contextError ? (
            <div className="text-center text-red-500">Erro de contexto: {String(contextError)}</div>
          ) : loading ? (
            ...
            reunioes.map((meeting: ReuniaoWithTCC, index: number) => (
              <div key={index} className="w-full bg-white border border-gray-300 rounded-lg p-4 shadow-sm">
                <h3 className="font-semibold text-lg text-gray-900">{meeting.titulo}</h3>
                {meeting.descricao && (
