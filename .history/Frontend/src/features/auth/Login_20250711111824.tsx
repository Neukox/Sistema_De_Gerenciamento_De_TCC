...
// Interface e função mockadas
export interface LoginData {
  email: string;
  password: string;
}

export async function mockado(data: LoginData): Promise<{ success: boolean }> {
...
function Login() {
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await mockado(loginData); // ✅ Usando a função mockada
  ...
  return (
    <div className="h-screen w-screen flex justify-center items-center bg-[#F3C50D]">
      <div className="bg-[#fffbef] w-[90%] max-w-[400px] md:max-w-[500px] lg:max-w-[600px] rounded-lg shadow-lg flex flex-col mt-1 p-1 px-4">
        ...
        <form className="flex flex-col mt-3 font-sans font-semibold" onSubmit={handleLogin} noValidate>
          {error && (
