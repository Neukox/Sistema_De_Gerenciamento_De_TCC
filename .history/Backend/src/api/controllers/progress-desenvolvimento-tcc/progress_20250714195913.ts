...
function calcularProgressoCompleto(tcc: any): ProgressoCalculado {
    ...
    const tarefasConcluidas = tcc.Atividades.filter((atividade: any) => atividade.status === "CONCLUIDA").length;
    ...
    const etapasConcluidas = tcc.EtapasTCC.filter((etapa: any) => etapa.status === "CONCLUIDA").length;
    ...
    const reunioesCumpridas = tcc.Reunioes.filter((reuniao: any) => reuniao.status === "REALIZADA").length;
