...
import { Atividade, EtapaTCC, Anotacao, Reuniao, Defesa } from "@prisma/client";
...
// Interface para TCC com todas as relações
interface TCCCompleto {
  id: number;
  titulo: string;
  status_atual: string;
  Aluno: {
...
export async function calculateTCCProgressController(
    ...
    try {
        ...
        const progresso = calcularProgressoCompleto(tcc as TCCCompleto);
                ...
                progresso: {
                    ...
                    detalhamento: {
                        tarefas: {
                            ...
                            concluidas: tcc.Atividades.filter(a => a.status === "CONCLUIDA").length
                        },
                        etapas: {
                            pontuacao: Math.round(progresso.progresso_etapas * 100) / 100,
                            peso: PESO_ETAPAS,
                            total: tcc.EtapasTCC.length,
                            concluidas: tcc.EtapasTCC.filter(e => e.status === "CONCLUIDA").length
                        },
                        anotacoes: {
                        ...
                        reunioes: {
                            ...
                            realizadas: tcc.Reunioes.filter(r => r.status === "REALIZADA").length
                        },
                        defesas: {
                            pontuacao: Math.round(progresso.progresso_defesas * 100) / 100,
                            peso: PESO_DEFESAS,
                            preBanca: tcc.Defesas.find(d => d.tipo === "PRE_BANCA")?.status === "REALIZADA",
                            bancaFinal: tcc.Defesas.find(d => d.tipo === "BANCA_FINAL")?.status === "REALIZADA"
                        }
                    }
                }
            }
...
function calcularProgressoCompleto(tcc: TCCCompleto): ProgressoCalculado {
    ...
    const totalTarefas = tcc.Atividades.length;
    const tarefasConcluidas = tcc.Atividades.filter((atividade: Atividade) => atividade.status === "CONCLUIDA").length;
    const progresso_tarefas = totalTarefas > 0 ? (tarefasConcluidas / totalTarefas) * PESO_TAREFAS : 0;

    // 2. Progresso das Etapas (30%)
    const totalEtapas = tcc.EtapasTCC.length;
    const etapasConcluidas = tcc.EtapasTCC.filter((etapa: EtapaTCC) => etapa.status === "CONCLUIDA").length;
    const progresso_etapas = totalEtapas > 0 ? (etapasConcluidas / totalEtapas) * PESO_ETAPAS : 0;

    // 3. Progresso das Anotações (10%)
    const qtdAnotacoes = tcc.Anotacoes.length;
    const progresso_anotacoes = Math.min((qtdAnotacoes / MIN_ANOTACOES_COMPLETAS), 1) * PESO_ANOTACOES;

    // 4. Progresso das Reuniões (10%)
    const reunioesAgendadas = tcc.Reunioes.length;
    const reunioesCumpridas = tcc.Reunioes.filter((reuniao: Reuniao) => reuniao.status === "REALIZADA").length;
    ...
    const preBanca = tcc.Defesas.find((defesa: Defesa) => defesa.tipo === "PRE_BANCA");
    const bancaFinal = tcc.Defesas.find((defesa: Defesa) => defesa.tipo === "BANCA_FINAL");
